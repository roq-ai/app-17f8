...
export function ChatbotListPage(props: ChatbotListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ChatbotInterface>>(
    () => `/chatbots?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ChatbotInterface) => {
    if (hasAccess('chatbot', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/chatbots/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/chatbots/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
